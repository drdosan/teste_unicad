var App = function () {
    ...
    var handleSidebar = function () {
        jQuery('.sidebar-menu .has-sub > a').click(function () {
            ...
            //raizenCoreJs.raizenMenus.UltimoItemSelecionado($(sub).parent().attr('id'));
        ...
        jQuery('.sidebar-collapse').click(function () {
            ...
            if (is_mobile) {
                ...
                if (collapsed) {
                    ...
                    collapsed = false;
                    $.cookie('mini_sidebar', '1', { path: '/', expires: 7300 });
                }
                else {
                    ...
                    collapsed = true;
                    $.cookie('mini_sidebar', '0', { path: '/', expires: 7300 });
                    handleMobileSidebar();
                }
            }
    ...
    var handleBoxTools = function () {
        //fa-angle-double-up
        $(document).on('click', '.box .tools .collapse, .box .tools .expand', function () {
            var el = jQuery(this).parents(".box").children(".box-body").first();
        ...
        /*Collapse All*/
        $(document).on('click', '.collapse-all, .expand-all', function () {
            var el = jQuery(".box").children(".box-body");
            if (jQuery(this).hasClass("collapse-all")) {
                jQuery(this).removeClass("collapse-all").addClass("expand-all");
                $(".collapse").removeClass("collapse").addClass("expand");
                $(".fa-chevron-up").removeClass("fa-chevron-up").addClass("fa-chevron-down");
                var i = jQuery(this).children(".fa-chevron-up-all");
                i.removeClass("fa-chevron-up-all").addClass("fa-chevron-down-all");
                jQuery(this).html(jQuery(this).html().replace('Recolher', 'Expandir'));
                el.slideUp(200);
            } else {
                jQuery(this).removeClass("expand-all").addClass("collapse-all");
                $(".expand").removeClass("expand").addClass("collapse");
                $(".fa-chevron-down").removeClass("fa-chevron-down").addClass("fa-chevron-up");
                var i = jQuery(this).children(".fa-chevron-down-all");
                i.removeClass("fa-chevron-down-all").addClass("fa-chevron-up-all");
                jQuery(this).html(jQuery(this).html().replace('Expandir', 'Recolher'));
                el.slideDown(200);
            }
        });
    ...
    var handleCalendar = function () {
        ...
        var addEvent = function (title) {
            title = title && title.length > 0 ? title : "Untitled Event";
            var html = $('<div class="external-event"></div>').text(title);
    ...
    var handleChat = function (elem) {
        var append = function () {
            ...
            var list = $('.chat-list', '.' + elem.replace(/[^a-zA-Z0-9-_]/g, ''));
            // Garante que `msg` é tratado como texto puro para evitar interpretações inseguras
            list.append(document.createTextNode(msg));
    ...
    var handleSliderNav = function () {
        ...
        $('#address-book .slider-content ul li ul li a').click(function (e) {
            ...
            var name = $(this).text();
            //Set the name
            $('#contact-card .panel-title').text(name);
            $('#contact-card #card-name').text(name);
            //Randomize the image
    ...
    /*-----------------------------------------------------------------------------------*/
    /*	Variaveis da classe App
	/*-----------------------------------------------------------------------------------*/
    var variaveis = function () {
        return {
            collapsed: collapsed,
            is_mobile: is_mobile,
            is_mini_menu: is_mini_menu,
...
(function (a, b) {
    a.fn.admin_tree = function (d) {
        ...
        this.each(function () {
            ...
            // Sanitização das classes de ícones usando regex para prevenir inserções inseguras
            var closeIcon = c["close-icon"] ? c["close-icon"].replace(/[<>]/g, "") : '';
            var unselectedIcon = c["unselected-icon"] ? c["unselected-icon"].replace(/[<>]/g, "") : '';

            // Criação dos elementos de forma segura
            var $treeFolder = $('<div>', { class: 'tree-folder', style: 'display:none;' });
            var $folderHeader = $('<div>', { class: 'tree-folder-header' });
            var $closeIcon = $('<i>', { class: closeIcon });
            var $folderName = $('<div>', { class: 'tree-folder-name' });
            var $folderContent = $('<div>', { class: 'tree-folder-content' });
            var $treeLoader = $('<div>', { class: 'tree-loader', style: 'display:none' });

            // Estrutura do "tree-folder"
            $folderHeader.append($closeIcon).append($folderName);
            $treeFolder.append($folderHeader).append($folderContent).append($treeLoader);

            // Estrutura do "tree-item"
            var $treeItem = $('<div>', { class: 'tree-item', style: 'display:none;' });
            if (unselectedIcon) {
                var $unselectedIcon = $('<i>', { class: unselectedIcon });
                $treeItem.append($unselectedIcon);
            }
            $treeItem.append($('<div>', { class: 'tree-item-name' }));

            // Inserção no elemento `e`
            e.empty().append($treeFolder).append($treeItem);

            // Adiciona classes com base na configuração
            e.addClass(c.selectable === true ? "tree-selectable" : "tree-unselectable");

            // Inicializa o componente `tree`
            e.tree(c);
        });
...
(function () {
    ...
    if (!Array.prototype.last) {
        Array.prototype.last = function () {
            var _idx = this.length;
            if (_idx > 0) return this[_idx - 1];
            else return null;
        }
    }
})(window);